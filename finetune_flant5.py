...
from peft import LoraConfig, get_peft_model, prepare_model_for_kbit_training, TaskType, set_peft_model_state_dict

dataset_path = 'data/expla_graph_predict_stance_train.json'
output_dir="flan-t5-xxl-lora-expla-graph-predict-stance"
val_size = 128
...
resume_and_save = False

if resume_from_checkpoint:
    checkpoint_path = ""
    # Check the available weights and load them
    if resume_and_save:
        checkpoint_name = os.path.join(checkpoint_path + '/checkpoint-400', "pytorch_model.bin")
    else:
        checkpoint_name = os.path.join(checkpoint_path, "adapter_model.bin")
